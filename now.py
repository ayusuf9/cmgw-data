...
colors_to_use = [cg_color_pallet['sapphire'], cg_color_pallet['ocean']]
...
# Callback for updating the combined chart
@app.callback(
...
def update_chart(country, security1, security2):
    # Filter data for both securities
    filtered_df = df[
        (df['country_exposure_name'] == country) &
        (df['securities'].isin([security1, security2]))
    ]
    
    # Get the latest observations for each year
    latest_obs = filtered_df.loc[
        filtered_df.groupby(['securities', filtered_df['month_date'].dt.year])['month_date'].idxmax()
    ]
    
    # Create the combined bubble chart
    fig = px.scatter(
        latest_obs,
        x='Year',
