...
class AgentState(BaseModel):
    ...
    processed: bool = False  # Flag to track if current message is processed
...
class FinancialQAAgent:
    ...
    def process_message(self, state: AgentState) -> AgentState:
        ...
        try:
            # If message is already processed, return state unchanged
            if state.processed:
                return state
            ...
            state.processed = True  # Mark as processed
            
            return state
            
        except Exception as e:
            state.error = str(e)
            state.processed = True  # Mark as processed even if there's an error
            return state

# Graph Construction
def should_continue(state: AgentState) -> bool:
    """Determine if the conversation should continue."""
    # Stop if message is processed, there's an error, or no messages
    return not state.processed and not state.error and len(state.messages) > 0
...
def main():
    ...
    initial_state = AgentState(
        ...
        processed=False  # Ensure initial state is not marked as processed
    )
    
    # Run graph and get config
    config = {"recursion_limit": 10}
    
    # Convert state to dict for graph input using model_dump()
...
# Create initial state with processed=False
initial_state = AgentState(
    messages=[HumanMessage(content="Your question here")],
    processed=False
)

# Convert state using model_dump() and run graph
state_dict = initial_state.model_dump()
final_state_dict = graph.invoke(state_dict, {"recursion_limit": 10})

# Get response
final_state = AgentState(**final_state_dict)
if not final_state.error and final_state.messages:
